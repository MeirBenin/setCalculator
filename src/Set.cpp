...
Set::Set(std::stringstream &ss) {
        int n,num ;
        if (!(ss >> n)) throw inputNotMatching("missing or not valid set size");
        if (n < 0) throw inputNotMatching("group size must be positive number ");
        for (auto i = 0; i < n; ++i) {
            if (!(ss >> num)) throw inputNotMatching("missing or not valid set element");
